...
# we only modify the codes below
class Seq2Seq(nn.Module):
	...
	def forward(self, src, trg):
		...
		enc_src = self.encoder(src.masked_fill(src >= self.encoder.tok_embedding.num_embeddings, 0), src_mask)  # replace OOV words with <UNK> before embedding

		#enc_src = [batch size, src len, hid dim]

		output, attention = self.decoder(trg.masked_fill(trg >= self.decoder.tok_embedding.num_embeddings, 0), enc_src, trg_mask, src_mask)
